function [Psat,Tsat,rhol,rhog] = SaturationStateGivenDensity(rhoSat)
    ...
    [delLt,delGt] = TriplePointReducedDensities();
	IsNotCritical = (delKn ~= 1);
    IsNotTriple   = any(bsxfun(@ne,delKn(:),[delLt,delGt]),2);
    ...
    if any(LiquidKnown)
        ...
        Calculate = (delLKn(:) >= DoNoIterationValueReducedDensityLiquid()) &...
                     IsNotCritical(LiquidKnown) & IsNotTriple(LiquidKnown);
    ...
    if any(GasKnown)
        ...
        Calculate = delGKn(:) <= DoNoIterationValueReducedDensityGas() &...
                     IsNotCritical(GasKnown) & IsNotTriple(GasKnown);
    ...
    rhol = real(delL .* rhoc    );
    rhog = real(delG .* rhoc    );
    Psat = real(Pressure(rhol,Tsat,false));
