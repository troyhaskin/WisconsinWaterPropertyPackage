function HelmDeriv = HelmholtzResidual_dd(delta,tau)
    ...
    for k = 8:51
        deltaMod2 = c(k) * delta.^(c(k))                                        ;
        Part      = (d(k) - deltaMod2).*(d(k) - 1 - deltaMod2) - c(k)*deltaMod2 ;
        Part      = delta.^(d(k)-2) .* tau.^(t(k)) .* Part                      ;
    ...
    for k = 52:54
        ...
        Part    = 4 * alpha(m)^2 .*(delta-epsilon(m)).^2 - 2 * alpha(m)         ;
        Part    = Part + d(k) * (d(k)-1) * delta.^(-2)                          ;
        Part    = Part - 4*d(k)*alpha(m) * delta.^(-1).*(delta-epsilon(m))      ;
